...
module.exports = {
        ...
        try {
            if (existRoom) {
                ...
                for (let i = 0; i < theStore.length; i++) {
                    const storeInfo = await Store.findById(
                        theStore[i].storeId
                    ).exec();
                    theStores.push(storeInfo);

                    const findImg = await Reivew.findOne({
                        storeId: theStores[i].storeId,
                        userId: theStores[i].userId,
                    }).exec();
                    TheImg.push(findImg ? findImg.imgURL : []);

                    const writerInfo = await User.findById(
                        theStores[i].userId
                    ).exec();
    ...
    inviteRoom: async (req, res) => {
        ...
        try {
            ...
            // 초대된 사람 제외
            const inviteGuest = await guestId.filter(
                (e) => !existUser.includes(e)
            );
            ...
            if (
                inviteGuest.length === 0 ||
                inviteGuest.includes(theRoom.ownerId)
            ) {
                return res
                    .status(400)
    ...
    deleteRoomStoreList: async (req, res) => {
        const { roomId, storeId } = req.params;
        try {
            // roomId와 storeId에 해당하는 Savelist 데이터 찾아서 삭제
            const removal = await Savelist.findOneAndDelete({
                roomId,
                storeId,
            });
            if (!removal) {
                return res
                    .status(400)
                    .send({
                        errorMessage: '맛방에 해당 맛집이 존재하지 않습니다.',
                    });
            }
            return res.status(200).send({
                result: true,
